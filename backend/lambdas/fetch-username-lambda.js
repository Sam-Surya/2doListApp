...
exports.handler = async (event) => {
    ...
    try {
        ...
        const [rows] = await connection.query('SELECT Name FROM 2todoListUsers WHERE Email = ?', [email]);

        if (rows.length > 0) {
            return {
                statusCode: 200,
                headers: responseHeaders,
                body: JSON.stringify({ name: rows[0].Name }),
            };
        } else {
            return {
                statusCode: 204,
                headers: responseHeaders,
                body: JSON.stringify({ message: 'No Name found for the provided email' }),
            };
        }
    } catch (error) {
